...
export class createemployee implements OnInit {
    ...
    ngOnInit() {
        ...
        if (this.employeeId > 0) {
            this.title = "Edit";
            this._empService.getEmployeeById(this.employeeId)
    ...
    save() {
        ...
        if (this.title == "Create") {
            this._empService.saveEmployee(this.employeeForm.value)
                .subscribe(data => {
                    this._router.navigate(['/fetch-employee']);
                }, error => this.errorMessage = error);
        } else if (this.title == "Edit") {
            this._empService.updateEmployee(this.employeeForm.value)
                .subscribe(data => {
                    this._router.navigate(['/fetch-employee']);
                }, error => this.errorMessage = error);
        }
